...
use gtk4::{
    ...
    Orientation, Switch, CssProvider,
};
...
use libadwaita::prelude::*;
use libadwaita::{Application as AdwApplication, HeaderBar};

fn main() {
    // Adwaita アプリケーションの作成
    let app = AdwApplication::builder()
        .application_id("com.example.HeaderbarColor")
        .build();

    app.connect_activate(|app| {
        // HeaderBar を作成
        // let header = HeaderBar::builder()
        //     .title_widget(&Label::new(Some("ヘッダー")))
        //     .show_end_title_buttons(true)
        //     .build();
...
fn main() {
    ...
    app.connect_activate(|app| {
        ...
        let header = HeaderBar::builder()
            .title_widget(&Label::new(Some("Luvita")))
            .show_end_title_buttons(true)
            .build();
        header.add_css_class("custom-header");
        ...
        let vbox = GtkBox::new(Orientation::Vertical, 0);
        vbox.append(&Label::new(Some("本文内容")));
...
// gtkのAppWindowにしたらエラーになったがタイトルバーは細くなった
// タイトルバーはadwaitaを使って色をかえられるようにするかも
...
fn main() {
    let app = Application::builder()
        .application_id("com.example.MouseFollow")
        .build();
    ...
    app.connect_activate(|app| {
        ...
        // 円の位置を保持する変数
        let circle_pos = Rc::new(RefCell::new((100.0, 100.0)));

        // DrawingAreaの作成
        let drawing_area = DrawingArea::new();
        let circle_pos_for_draw = circle_pos.clone();

        drawing_area.set_draw_func(move |_, cr, _, _| {
            let (x, y) = *circle_pos_for_draw.borrow();
...
// マウスの右クリックされた位置と離された位置を表示
use gtk4::prelude::*;
use gtk4::{Application, ApplicationWindow, DrawingArea, EventControllerMotion, GestureClick};
use std::cell::RefCell;
use std::rc::Rc;

fn main() {
    let app = Application::builder()
        .application_id("com.example.MouseFollower")
        .build();

    app.connect_activate(|app| {
        let window = ApplicationWindow::builder()
            .application(app)
            .title("Right Click Follower")
            .default_width(800)
            .default_height(600)
            .build();

        let circle_pos = Rc::new(RefCell::new((100.0, 100.0))); // 初期位置
        let is_right_click_pressed = Rc::new(RefCell::new(false)); // 右クリックの状態
        ...
        drawing_area.set_draw_func({
            let circle_pos = circle_pos.clone(); // circle_pos の参照をクロージャ内に渡す
            move |_, cr, _, _| {
                let (x, y) = *circle_pos.borrow();
                cr.set_source_rgb(0.2, 0.8, 1.0); // 円の色
                cr.arc(x, y, 20.0, 0.0, std::f64::consts::PI * 2.0); // 円を描く
                cr.fill().unwrap();
            }
        });
        ...
        click_gesture.connect_pressed({
            let is_right_click_pressed = is_right_click_pressed.clone();
            move |_, _, _, _| {
                // 右クリックが押されたら、円を追従させる
                *is_right_click_pressed.borrow_mut() = true; // 右クリック中
        ...
        click_gesture.connect_released({
            ...
            move |_, _, _, _| {
                // 右クリックが離されたら、追従を停止
                *is_right_click_pressed.borrow_mut() = false; // 右クリックが離された
                println!("Right click released!");
            }
        });

        // 動きの監視（右クリックされている間だけ動く）
        ...
        motion_controller.connect_motion({
            ...
            let is_right_click_pressed = is_right_click_pressed.clone();
            move |_, x, y| {
                if *is_right_click_pressed.borrow() {
                    // 右クリックされている間だけ動く
                    *circle_pos.borrow_mut() = (x, y);
                    println!("Circle moved to: ({}, {})", x, y); // 動きが反映されているかデバッグ
                }
            }
        });

        // イベントコントローラーを追加
        drawing_area.add_controller(click_gesture); // GestureClick
        drawing_area.add_controller(motion_controller); // Motion controller
